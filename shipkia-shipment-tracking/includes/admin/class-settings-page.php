...
class Shipkia_Settings_Page
{
    ...
    public static function render_enabled_field()
    {
        ...
        ?>
        <label><input type="checkbox" name="shipkia_tracking_enabled" value="yes" <?php if (function_exists('checked')) checked($val, 'yes'); ?> />
            <?php if (function_exists('_e')) _e('Enable Shipkia Tracking', 'shipkia-shipment-tracking'); else echo 'Enable Shipkia Tracking'; ?>
        </label>
        <?php
    }
    ...
    public static function render_text_field()
    {
        ...
        ?>
        <input type="text" name="shipkia_tracking_button_text" value="<?php echo function_exists('esc_attr') ? esc_attr($val) : $val; ?>" class="regular-text" />
        <?php
    }
    ...
    public static function render_new_tab_field()
    {
        ...
        ?>
        <label><input type="checkbox" name="shipkia_tracking_new_tab" value="yes" <?php if (function_exists('checked')) checked($val, 'yes'); ?> />
            <?php if (function_exists('_e')) _e('Open tracking link in a new tab', 'shipkia-shipment-tracking'); else echo 'Open tracking link in a new tab'; ?>
        </label>
        <?php
    }
    ...
    public static function render_multisite_field()
    {
        ...
        <div style="max-height: 200px; overflow-y: auto; border: 1px solid #ddd; padding: 10px; background: #fff;">
            ...
            <label style="display: block; margin-bottom: 5px;">
                <input type="checkbox" name="shipkia_multisite_sites[]" value="<?php echo function_exists('esc_attr') ? esc_attr($site->blog_id) : $site->blog_id; ?>" <?php echo $checked; ?> /> 
                <?php 
                if ($site_details) {
        ...
        <p class="description"><?php if (function_exists('_e')) _e('Select the sites you want to connect to Shipkia.', 'shipkia-shipment-tracking'); else echo 'Select the sites you want to connect to Shipkia.'; ?></p>
        <?php
    }
        ...
        <div id="shipkia-connection-status">
            <?php if ($show_activation_notice && !$is_connected): ?>
                <div class="notice notice-info inline" style="padding: 15px; border-left: 4px solid #2271b1; margin-bottom: 20px; background: #fff;">
                    <h3 style="margin-top: 0; color: #2271b1;">🚀 <?php if (function_exists('_e')) _e('Ready to Connect!', 'shipkia-shipment-tracking'); else echo 'Ready to Connect!'; ?></h3>
                    <p><?php if (function_exists('_e')) _e('Shipkia Shipment Tracking plugin has been activated. Please initialize the connection to start syncing your orders and products.', 'shipkia-shipment-tracking'); else echo 'Shipkia Shipment Tracking plugin has been activated. Please initialize the connection to start syncing your orders and products.'; ?></p>
                    <button type="button" id="shipkia-initialize-btn" class="button button-primary button-large" style="margin-top: 10px;">
                        <?php if (function_exists('_e')) _e('Initialize Store Connection', 'shipkia-shipment-tracking'); else echo 'Initialize Store Connection'; ?>
                    </button>
                ...
                <?php if ($is_connected): ?>
                    ...
                    <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
                        <tr>
                            ...
                            <td style="padding: 5px 0;"><?php echo function_exists('esc_html') ? esc_html(ucfirst($status['platform'] ?: 'WooCommerce')) : ucfirst($status['platform'] ?: 'WooCommerce'); ?></td>
                        </tr>
                        <?php if ($status['connected_domain']): ?>
                        <tr>
                            <td style="padding: 5px 0;"><strong>Store Domain:</strong></td>
                            <td style="padding: 5px 0;"><code><?php echo function_exists('esc_html') ? esc_html($status['connected_domain']) : $status['connected_domain']; ?></code></td>
                        </tr>
                        <?php endif; ?>
                        ...
                        <tr>
                            ...
                            <td style="padding: 5px 0;">
                                <a href="<?php echo function_exists('esc_url') ? esc_url($status['shipkia_url']) : $status['shipkia_url']; ?>" target="_blank" style="text-decoration: none;">
                                    <code style="color: #0073aa;"><?php echo function_exists('esc_html') ? esc_html($status['shipkia_url']) : $status['shipkia_url']; ?></code>
                        ...
                        <tr>
                            ...
                            <td style="padding: 5px 0;"><code><?php echo function_exists('esc_html') ? esc_html($status['store_id']) : $status['store_id']; ?></code></td>
                        </tr>
                        <tr>
                    ...
                    <p style="margin: 15px 0 0 0; display: flex; gap: 10px;">
                        <?php if ($status['is_active']): ?>
                            ...
                            ?>
                                <a href="<?php echo function_exists('esc_url') ? esc_url($sync_url) : $sync_url; ?>" id="shipkia-sync-order-product-link" target="_blank" class="button button-secondary">
    ...
    public static function render_app_url_field()
    {
        ...
        <div class="shipkia-app-url-container">
            <input type="text" id="shipkia_app_url" name="shipkia_app_url" value="<?php echo function_exists('esc_attr') ? esc_attr($val) : $val; ?>" 
                   class="regular-text" <?php echo $is_connected ? 'disabled readonly' : ''; ?> 
                   placeholder="https://app.shipkia.com" />
            
            <?php if (!$is_connected): ?>
                <p class="description"><?php if (function_exists('_e')) _e('Enter your Shipkia platform URL (e.g., https://app.shipkia.com)', 'shipkia-shipment-tracking'); else echo 'Enter your Shipkia platform URL (e.g., https://app.shipkia.com)'; ?></p>
                <button type="button" id="shipkia-connect-btn" class="button button-primary" style="margin-top: 10px;">
                    <?php if (function_exists('_e')) _e('Connect to Shipkia', 'shipkia-shipment-tracking'); else echo 'Connect to Shipkia'; ?>
                </button>
                    ...
                    $('#shipkia-connect-btn').on('click', function() {
                        ...
                        $.post(ajaxurl, {
                            ...
                            if (response.success) {
                                ...
                                $btn.prop('disabled', false).text('<?php if (function_exists('_e')) _e('Connect to Shipkia', 'shipkia-shipment-tracking'); else echo 'Connect to Shipkia'; ?>');
                            }
                        });
                    });
                });
            ...
            <?php else: ?>
                <p class="description"><?php if (function_exists('_e')) _e('Disconnect above to change the Shipkia URL', 'shipkia-shipment-tracking'); else echo 'Disconnect above to change the Shipkia URL'; ?></p>
            <?php endif; ?>
        </div>
        <?php
    }
